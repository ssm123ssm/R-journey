...
ggplot(data = murders, aes(x = population/10^6, y = total)) + 
  ...
  theme(
    axis.title.x = element_text(margin = margin(t = 10)),  # adjust margin for x-axis label
...
p_an <- gapminder  %>% ggplot(aes(x = gdp, y=life_expectancy, size = population, colour = continent)) +
  geom_point( alpha = 0.7) +
  scale_size(range = c(2, 12)) +
  scale_x_log10() +
  labs(x = "GDP per capita", y = "Life expectancy") +
...
#save the animation
#anim_save("gapminder.gif", animate(p_an, nframes = 100), path = "anim")
...
dslabs::gapminder %>% filter(year == 2000) %>%
    mutate(continent = reorder(continent, gdp, FUN = median)) %>%
